...
public class ExportPlugin extends PluginExport
{
	...
	public void onResume()
	{
		...
		{
			View v = LayoutInflater.from(CameraScreenActivity.getMainContext()).inflate(R.layout.plugin_export_gps, null);
			gpsInfoImage = (RotateImageView) v.findViewById(R.id.gpsInfoImage);
			gpsInfoImage.setImageDrawable(CameraScreenActivity.getMainContext().getResources().getDrawable(R.drawable.gps_off));

			addInfoView(gpsInfoImage);

			MLocation.subsribe(CameraScreenActivity.getInstance());
	...
	public void ShowGPSStatus(int event)
	{
		...
		case GpsStatus.GPS_EVENT_STARTED:
			gpsInfoImage
					.setImageDrawable(CameraScreenActivity.getMainContext().getResources().getDrawable(R.drawable.gps_search));
			gpsInfoImage.setVisibility(View.VISIBLE);
			break;
		case GpsStatus.GPS_EVENT_SATELLITE_STATUS:
			if (!isFirstGpsFix)
				return;
			gpsInfoImage.setImageDrawable(CameraScreenActivity.getMainContext().getResources().getDrawable(R.drawable.gps_found));
			gpsInfoImage.setVisibility(View.VISIBLE);
			isFirstGpsFix = false;
			break;
		default:
