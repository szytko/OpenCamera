...
public class BarcodeScannerVFPlugin extends PluginViewfinder
{
	...
	public void onOrientationChanged(int orientation)
	{
		...
		{
			mBarcodesListButton.setOrientation(CameraScreenActivity.getGUIManager().getLayoutOrientation());
			mBarcodesListButton.invalidate();
			mBarcodesListButton.requestLayout();
		}
		if (barcodeHistoryDialog != null)
		{
			barcodeHistoryDialog.setRotate(CameraScreenActivity.getGUIManager().getLayoutOrientation());
		}
		if (barcodeViewDialog != null)
		{
			barcodeViewDialog.setRotate(CameraScreenActivity.getGUIManager().getLayoutOrientation());
		}
	}
	...
	{
		mSoundPlayer = new SoundPlayer(CameraScreenActivity.getMainContext(), CameraScreenActivity.getMainContext().getResources()
	...
	public void createBoundView()
	{
		...
		mBound = new BoundingView(CameraScreenActivity.getMainContext());
		mBound.setVisibility(View.VISIBLE);

		RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(LayoutParams.MATCH_PARENT,
				LayoutParams.MATCH_PARENT);
		((RelativeLayout) CameraScreenActivity.getInstance().findViewById(R.id.specialPluginsLayout)).addView(mBound, params);

		mBound.setLayoutParams(params);

		((RelativeLayout) CameraScreenActivity.getInstance().findViewById(R.id.specialPluginsLayout)).requestLayout();
	}
	...
	public void createScreenButton()
	{
		...
		CameraScreenActivity.getGUIManager().removeViews(mButtonsLayout, R.id.specialPluginsLayout3);
		...
		((RelativeLayout) CameraScreenActivity.getInstance().findViewById(R.id.specialPluginsLayout3)).addView(mButtonsLayout,
				params);

		mButtonsLayout.setLayoutParams(params);
		mButtonsLayout.requestLayout();

		((RelativeLayout) CameraScreenActivity.getInstance().findViewById(R.id.specialPluginsLayout3)).requestLayout();
	...
	{
		barcodeHistoryDialog = new BarcodeHistoryListDialog(CameraScreenActivity.getInstance());
		barcodeHistoryDialog.setRotate(CameraScreenActivity.getGUIManager().getLayoutOrientation());
	...
	public void onPreviewFrame(byte[] data)
	{
		...
		new DecodeAsyncTask(CameraScreenActivity.getPreviewWidth(), CameraScreenActivity.getPreviewHeight()).execute(data);
	...
	protected void showBarcodeViewDialog(Barcode barcode)
	{
		try
		{
			barcodeViewDialog = new BarcodeViewDialog(CameraScreenActivity.getInstance(), barcode);
			barcodeViewDialog.setRotate(CameraScreenActivity.getGUIManager().getLayoutOrientation());
			showGUI();
	...
	public final synchronized Rect getBoundingRect()
	{
		...
		int height = (int) (CameraScreenActivity.getPreviewHeight() * heightFraction);
		int width = (int) (CameraScreenActivity.getPreviewWidth() * widthFraction);
		int left = (int) (CameraScreenActivity.getPreviewWidth() * ((1 - widthFraction) / 2));
		int top = (int) (CameraScreenActivity.getPreviewHeight() * ((1 - heightFraction) / 2));
	...
	private synchronized File saveDecodedImageToFile(byte[]... datas)
	{
		...
		ImageConversion.TransformNV21(datas[0], dataRotated, CameraScreenActivity.getPreviewWidth(),
				CameraScreenActivity.getPreviewHeight(), 0, 0, 1);
		datas[0] = dataRotated;

		Rect rect = new Rect(0, 0, CameraScreenActivity.getPreviewHeight(), CameraScreenActivity.getPreviewWidth());
		YuvImage img = new YuvImage(datas[0], ImageFormat.NV21, CameraScreenActivity.getPreviewHeight(),
				CameraScreenActivity.getPreviewWidth(), null);
		...
		if (os != null)
		{
			try
			{
				...
				bitmap = Bitmap.createBitmap(bitmap, 0, 0, CameraScreenActivity.getPreviewHeight(), CameraScreenActivity.getPreviewWidth(),
						matrix, false);
