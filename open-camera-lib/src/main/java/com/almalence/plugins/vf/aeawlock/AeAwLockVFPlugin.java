...
public class AeAwLockVFPlugin extends PluginViewfinder
{
	...
	public void onGUICreate()
	{
		...
		if (showAEAWLock)
		{
			...
			CameraScreenActivity.getGUIManager().removeViews(buttonsLayout, R.id.specialPluginsLayout2);

			RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(LayoutParams.MATCH_PARENT,
					LayoutParams.WRAP_CONTENT);
			params.height = (int) CameraScreenActivity.getAppResources().getDimension(R.dimen.aeawlock_size);

			params.addRule(RelativeLayout.ALIGN_PARENT_BOTTOM);

			((RelativeLayout) CameraScreenActivity.getInstance().findViewById(R.id.specialPluginsLayout2)).addView(
					this.buttonsLayout, params);

			this.buttonsLayout.setLayoutParams(params);
			this.buttonsLayout.requestLayout();

			((RelativeLayout) CameraScreenActivity.getInstance().findViewById(R.id.specialPluginsLayout2)).requestLayout();
...
//			awLockButton.requestLayout();
	...
	private void AeLock()
	{
		...
		if (params != null)
		{
			if (CameraController.isExposureLockSupported())
			{
				...
				Drawable icon = CameraScreenActivity.getMainContext().getResources().getDrawable(icon_ae_lock);
	...
	private void AwLock()
	{
		...
		if (params != null)
		{
			if (CameraController.isWhiteBalanceLockSupported())
			{
				...
				Drawable icon = CameraScreenActivity.getMainContext().getResources().getDrawable(icon_aw_lock);
	...
	private void AeUnlock()
	{
		...
		if (params != null)
		{
			...
			Drawable icon = CameraScreenActivity.getMainContext().getResources().getDrawable(icon_ae_unlock);
			aeLockButton.setImageDrawable(icon);
	...
	private void AwUnlock()
	{
		...
		if (params != null)
		{
			...
			Drawable icon = CameraScreenActivity.getMainContext().getResources().getDrawable(icon_aw_unlock);
			awLockButton.setImageDrawable(icon);
	...
	public void onOrientationChanged(int orientation)
	{
		...
		{
			aeLockButton.setOrientation(CameraScreenActivity.getGUIManager().getLayoutOrientation());
			aeLockButton.invalidate();
			aeLockButton.requestLayout();
		}
		if (awLockButton != null)
		{
			awLockButton.setOrientation(CameraScreenActivity.getGUIManager().getLayoutOrientation());
			awLockButton.invalidate();
			awLockButton.requestLayout();
		}
	}
	...
	public void onPreferenceCreate(PreferenceFragment prefActivity)
	{
		...
		if (cat != null && !CameraScreenActivity.getGUIManager().mEVLockSupported && !CameraScreenActivity.getGUIManager().mWBLockSupported)
		{
