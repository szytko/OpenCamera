...
public class ZoomVFPlugin extends PluginViewfinder
{
	...
	{
		zoomStopping = true;
		CameraScreenActivity.getGUIManager().removeViews(zoomPanel, R.id.specialPluginsLayout);
	}
	...
	public void onGUICreate()
	{
		...
		RelativeLayout.LayoutParams lp = (RelativeLayout.LayoutParams) CameraScreenActivity.getInstance()
				.findViewById(R.id.specialPluginsLayout).getLayoutParams();
		mainLayoutHeight = lp.height;

		zoomPanelWidth = CameraScreenActivity.getAppResources().getDrawable(R.drawable.scrubber_control_pressed_holo)
				.getMinimumWidth();
		...
		((RelativeLayout) CameraScreenActivity.getInstance().findViewById(R.id.specialPluginsLayout)).addView(this.zoomPanel,
				params);
...
//				Log.d("ZoomPlugin", "onAnimationEnd");
				if (zoomStopping)
				{
					...
					RelativeLayout specialLayout = (RelativeLayout) CameraScreenActivity.getInstance().findViewById(
