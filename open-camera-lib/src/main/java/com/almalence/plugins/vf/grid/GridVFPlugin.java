...
public class GridVFPlugin extends PluginViewfinder
{
	...
	public void onGUICreate()
	{
		...
		if (grid == null)
			grid = new ImageView(CameraScreenActivity.getMainContext());
	...
	public void onQuickControlClick()
	{
		...
		CameraScreenActivity.getGUIManager().removeViewQuick(grid);
		...
		try
		{
			...
			grid.setScaleType(ScaleType.FIT_XY);
			clearViews();
			CameraScreenActivity.getGUIManager().addViewQuick(grid, Plugin.ViewfinderZone.VIEWFINDER_ZONE_FULLSCREEN);
		} catch (Exception e)
		{
			e.printStackTrace();
	...
	private void setProperGrid()
	{
		...
		CameraController.Size previewSize = new CameraController.Size(CameraScreenActivity.getPreviewWidth(),
		...
		if (0 == gridType)
		{
			...
			grid.setImageDrawable(CameraScreenActivity.getAppResources().getDrawable(resID));
		} else if (1 == gridType)
		{
			switch (ri)
			{
		...
		{
			grid.setImageDrawable(CameraScreenActivity.getAppResources()
