...
public class MainScreen_old extends Activity implements ApplicationInterface, OnClickListener, View.OnTouchListener,
		SurfaceHolder.Callback, Handler.Callback, Camera.ShutterCallback
{
	...
	public static MainScreen_old thiz;
	...
	public void onCreate(Bundle savedInstanceState)
	{
		...
		SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(MainScreen_old.getMainContext());
		...
		CameraController.onCreate(MainScreen_old.thiz, MainScreen_old.thiz, PluginManager.getInstance());
		...
		orientListener = new OrientationEventListener(this)
		{
			...
			public void onOrientationChanged(int orientation)
			{
				...
				if (MainScreen_old.thiz.landscapeIsNormal)
				{
					orientation += 90;
				}
				...
				{
					if (MainScreen_old.wantLandscapePhoto)
					{
						MainScreen_old.wantLandscapePhoto = false;
					}
				} else
				{
					if (!MainScreen_old.wantLandscapePhoto)
					{
						MainScreen_old.wantLandscapePhoto = true;
					}
				}
		...
		screenTimer = new CountDownTimer(180000, 180000)
		{
			...
			{
				boolean isVideoRecording = PreferenceManager.getDefaultSharedPreferences(MainScreen_old.getMainContext())
		...
		if (this.getIntent().getAction() != null)
		{
			if (this.getIntent().getAction().equals(MediaStore.ACTION_IMAGE_CAPTURE))
			{
				try
				{
					...
					MainScreen_old.setForceFilename(new File(((Uri) forceFilenameUri).getPath()));
					if (MainScreen_old.getForceFilename().getAbsolutePath().equals("/scrapSpace"))
					{
						MainScreen_old.setForceFilename(new File(Environment.getExternalStorageDirectory()
                                .getAbsolutePath() + "/mms/scrapSpace/.temp.jpg"));
						new File(MainScreen_old.getForceFilename().getParent()).mkdirs();
					}
				} catch (Exception e)
				{
					MainScreen_old.setForceFilename(null);
				}
			} else
			{
				MainScreen_old.setForceFilename(null);
		...
		if (goShopping)
		{
			if (MainScreen_old.thiz.titleUnlockAll == null || MainScreen_old.thiz.titleUnlockAll.endsWith("check for sale"))
			{
				Toast.makeText(MainScreen_old.getMainContext(),
	 ...
	 */
	public static MainScreen_old getInstance()
	{
		return thiz;
	}
	...
	private void setImageSizeOptions(PreferenceFragment prefActivity, int mode)
	{
		...
		if (mode == MODE_GENERAL)
		{
			...
			currentIdx = Integer.parseInt(MainScreen_old.getImageSizeIndex());
		...
		} else if (mode == MODE_SMART_MULTISHOT_AND_NIGHT)
		{
			...
			currentIdx = Integer.parseInt(CameraController.MultishotResolutionsIdxesList.get(MainScreen_old
	...
	public void onAdvancePreferenceCreate(PreferenceFragment prefActivity)
	{
		...
		final CheckBoxPreference fp = (CheckBoxPreference) prefActivity.findPreference(MainScreen_old.sCaptureRAWPref);
		...
		if (cp != null)
		{
			...
			cp.setOnPreferenceChangeListener(new OnPreferenceChangeListener()
			{
				...
				{
					PreferenceManager.getDefaultSharedPreferences(MainScreen_old.getMainContext()).edit()
							.putBoolean(MainScreen_old.sInitModeListPref, true).commit();
					...
					{
						PreferenceManager.getDefaultSharedPreferences(MainScreen_old.getMainContext()).edit()
								.putBoolean(MainScreen_old.sCaptureRAWPref, false).commit();
						fp.setEnabled(false);
					}
	...
	protected void onStart()
	{
		...
		MainScreen_old.getGUIManager().onStart();
	...
	protected void onStop()
	{
		...
		MainScreen_old.getGUIManager().onStop();
	...
	protected void onDestroy()
	{
		...
		prefs.edit().putBoolean(MainScreen_old.sPhotoTimeLapseIsRunningPref, false);
		prefs.edit().putBoolean(MainScreen_old.sPhotoTimeLapseActivePref, false);

		MainScreen_old.getGUIManager().onDestroy();
	...
	protected void onResume()
	{
			...
			new CountDownTimer(50, 50)
			{
				...
				public void onFinish()
				{
					SharedPreferences prefs = PreferenceManager
							.getDefaultSharedPreferences(MainScreen_old.getMainContext());
					...
					saveToPath = prefs.getString(sSavePathPref, Environment.getExternalStorageDirectory()
							.getAbsolutePath());
					saveToPreference = prefs.getString(MainScreen_old.sSaveToPref, "0");
					sortByDataPreference = prefs.getBoolean(MainScreen_old.sSortByDataPref, false);
					...
					captureRAW = prefs.getBoolean(MainScreen_old.sCaptureRAWPref, false);


					MainScreen_old.getGUIManager().onResume();
					PluginManager.getInstance().onResume();
					CameraController.onResume();
					MainScreen_old.thiz.mPausing = false;

					if ((surfaceCreated && (!CameraController.isCameraCreated())) ||
							(surfaceCreated && MainScreen_old.getInstance().getSwitchingMode()))
					{
						MainScreen_old.thiz.findViewById(R.id.mainLayout2).setVisibility(View.VISIBLE);
		...
		if (memoryFree < 30)
			Toast.makeText(MainScreen_old.getMainContext(), "Almost no free space left on internal storage.",
	...
	{
		SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(MainScreen_old.getMainContext());
		CameraController.setCameraIndex(!prefs.getBoolean(MainScreen_old.sUseFrontCameraPref, false) ? 0 : 1);
		shutterPreference = prefs.getBoolean(MainScreen_old.sShutterPref, false);
		shotOnTapPreference = Integer.parseInt(prefs.getString(MainScreen_old.sShotOnTapPref, "0"));
		imageSizeIdxPreference = prefs.getString(CameraController.getCameraIndex() == 0 ? MainScreen_old.sImageSizeRearPref
				: MainScreen_old.sImageSizeFrontPref, "-1");
	...
	protected void onPause()
	{
		...
		MainScreen_old.getGUIManager().onPause();
		...
		{
			AudioManager mgr = (AudioManager) MainScreen_old.thiz.getSystemService(MainScreen_old.AUDIO_SERVICE);
		...
		} else if (!isCreating)
		{
			new CountDownTimer(50, 50)
			{
				...
				public void onFinish()
				{
					...
					if (!MainScreen_old.thiz.mPausing && surfaceCreated && (!CameraController.isCameraCreated()))
					{
						MainScreen_old.thiz.findViewById(R.id.mainLayout2).setVisibility(View.VISIBLE);
	...
	private void onCameraConfigured()
	{
		...
		new CountDownTimer(10, 10)
		{
			...
			public void onFinish()
			{
					...
					{
						Toast.makeText(MainScreen_old.thiz, "Unable to start camera", Toast.LENGTH_LONG).show();
						return;
					}
				...
				MainScreen_old.mApplicationStarted = true;

				if (MainScreen_old.isForceClose)
	...
	public boolean onKeyDown(int keyCode, KeyEvent event)
	{
		...
		{
			MainScreen_old.getGUIManager().onHardwareShutterButtonPressed();
			return true;
		}
		...
		if (keyCode == KeyEvent.KEYCODE_FOCUS)
		{
			...
			{
				MainScreen_old.getGUIManager().onHardwareFocusButtonPressed();
			}
			return true;
		}
		...
		{
			SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(MainScreen_old.getMainContext());
			int buttonFunc = Integer.parseInt(prefs.getString(MainScreen_old.sVolumeButtonPref, "0"));
			...
			{
				MainScreen_old.getGUIManager().onVolumeBtnExpo(keyCode);
				return true;
	...
	{
		if (mApplicationStarted)
			MainScreen_old.getGUIManager().onClick(v);
	}
	...
	{
		if (mApplicationStarted)
			return MainScreen_old.getGUIManager().onTouch(view, event);
		return true;
	}
	...
	{
		MainScreen_old.getGUIManager().onButtonClick(v);
	}
		...
		case PluginManager.MSG_CAMERA_READY:
			{
				if (CameraController.isCameraCreated())
				{
					...
					MainScreen_old.getGUIManager().onGUICreate();
				}
			}
			break;
	...
	public void showOpenGLLayer(final int version)
	{
		if (glView == null)
		{
			glView = new GLLayer(MainScreen_old.getMainContext(), version);
			LayoutParams params = MainScreen_old.getPreviewSurfaceView().getLayoutParams();
	...
	{
		if (!MainScreen_old.isShutterSoundEnabled())
		{
			MediaPlayer mediaPlayer = MediaPlayer.create(MainScreen_old.thiz, sound);
			mediaPlayer.start();
		}
	}
	...
	{
		return MainScreen_old.thiz.getResources();
	}
	...
	private void resetOrSaveSettings()
	{
		...
		if (!isSaving)
		{
			prefsEditor.putInt(MainScreen_old.sEvPref, 0);
			prefsEditor.commit();
		}
		...
		if (!isSaving)
		{
			prefsEditor.putInt(MainScreen_old.sDelayedCapturePref, 0);
			prefsEditor.putBoolean(MainScreen_old.sSWCheckedPref, false);
			prefsEditor.putBoolean(MainScreen_old.sDelayedFlashPref, false);
			prefsEditor.putBoolean(MainScreen_old.sDelayedSoundPref, false);
			prefsEditor.putInt(MainScreen_old.sDelayedCaptureIntervalPref, 0);
		...
		{
			prefsEditor.putInt(MainScreen_old.sPhotoTimeLapseCaptureIntervalPref, 0);
			prefsEditor.putInt(MainScreen_old.sPhotoTimeLapseCaptureIntervalMeasurmentPref, 0);
			prefsEditor.putBoolean(MainScreen_old.sPhotoTimeLapseIsRunningPref, false);
			prefsEditor.putBoolean(MainScreen_old.sPhotoTimeLapseActivePref, false);
